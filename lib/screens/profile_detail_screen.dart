...
class _ProfileDetailScreenState extends State<ProfileDetailScreen> {
  ...
  final List<String> _monthNames = const ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        _displayNameController.text = (data['displayName'] ?? _auth.currentUser?.displayName ?? '').toString();
        dateOfBirth = data['dateOfBirth'] != null ? (data['dateOfBirth'] as Timestamp).toDate() : null;
        gender = (data['gender'] ?? '').toString();
        mbtiType = (data['mbtiType'] ?? '').toString();
        profileImage = (data['profileImage'] ?? '').toString();
        final interests = data['interests'] != null ? List<String>.from(data['interests']) : <String>[];
  ...
  Future<void> _pickProfileImage() async {
    ...
    if (!status.isGranted) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Permission denied to access photos')));
      }
      return;
    }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (_newImageFile != null) {
        final storageRef = FirebaseStorage.instance.ref().child('prof_pics/${widget.userId}.jpg');
        ...
        child: Stack(children: [
          CircleAvatar(
            ...
            backgroundImage: _newImageFile != null
                ...
                : (profileImage.isNotEmpty ? NetworkImage(profileImage) : null) as ImageProvider<Object>?,
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(color: Color(0xff545454), shape: BoxShape.circle),
  ...
  Widget _buildDOBFields() {
    ...
    final days = (_selectedYear != null && _selectedMonth != null)
        ? List.generate(DateTime(_selectedYear!, _selectedMonth! + 1, 0).day, (i) => i + 1)
          ...
          _buildMonthDropdown(_selectedMonth, 'Month', (val) {
            setState(() {
              ...
              if (_selectedDay != null && val != null) {
                final maxDay = DateTime(_selectedYear ?? 2000, val + 1, 0).day;
                if (_selectedDay! > maxDay) _selectedDay = null;
              }
            });
          }),
          const SizedBox(width: 8),
          _buildIntDropdown(_selectedDay, 'Day', days, (val) => setState(() => _selectedDay = val)),
          const SizedBox(width: 8),
          _buildIntDropdown(_selectedYear, 'Year', years, (val) {
            setState(() {
              _selectedYear = val;
              if (_selectedMonth != null && _selectedDay != null && val != null) {
  ...
  Widget _buildMonthDropdown(int? currentValue, String hint, ValueChanged<int?> onChanged) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12),
        decoration: BoxDecoration(color: Colors.black.withOpacity(0.3), borderRadius: BorderRadius.circular(8)),
        child: DropdownButton<int>(
          ...
          items: List.generate(12, (i) => i + 1)
              .map((m) => DropdownMenuItem(value: m, child: Text(_monthNames[m - 1], style: const TextStyle(color: Colors.white))))
              .toList(),
          onChanged: onChanged,
        ),
      ),
  ...
  Widget _buildIntDropdown(int? currentValue, String hint, List<int> items, ValueChanged<int?> onChanged) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12),
        decoration: BoxDecoration(color: Colors.black.withOpacity(0.3), borderRadius: BorderRadius.circular(8)),
        child: DropdownButton<int>(
          ...
          isExpanded: true,
          underline: const SizedBox(),
          items: items.map((d) => DropdownMenuItem(value: d, child: Text(d.toString(), style: const TextStyle(color: Colors.white)))).toList(),
          onChanged: onChanged,
        ),
      ),
    );
        ...
        ...categorizedInterests.entries.map((entry) {
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(
              ...
              child: Text(category, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
            ),
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: interests.map((interest) {
                ...
                final color = isSelected ? darkenColor(getColorForInterest(interest)) : const Color(0xff424242);
                    ...
                    setState(() {
                      if (isSelected) {
                        ...
                        if (_selectedInterests.length >= 8) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('You can only choose up to 8 interest tags.')));
                        } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Profile Info', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 18, color: Colors.white)),
        actions: [
          if (!_isLoading)
            TextButton(
                      ...
                      Card(
                        ...
                        child: Padding(
                          ...
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            const Text('Basic Info', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 16),
                            _buildDOBFields(),
                            _buildDropDownField('Gender', gender, const ['Male', 'Female', 'Other'], onChanged: (v) => setState(() => gender = v ?? '')),
                            _buildDropDownField('MBTI Type', mbtiType, _mbtiTypes, onChanged: (v) => setState(() => mbtiType = v ?? '')),
                            const SizedBox(height: 20),
